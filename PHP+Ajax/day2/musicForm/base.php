...
function musicTest () {
    ...
    if ($_FILES['cover']['error']!=UPLOAD_ERR_OK)
    {
        ...
        if (strcmp($name_jpg,'.jpg')!=0)
        {
            ...
            return;
        }else if (!move_uploaded_file($_FILES['cover']['tmp_name'],'img/'.$_FILES['cover']['name']))
        {
            $GLOBALS["bool_submit"]="添加失败，请重新上传海报";
            return;
        }
    ...
    if ($_FILES['mp3']['error']!=UPLOAD_ERR_OK)
    {
        ...
        if (strcmp($name_mp3,'.mp3')!=0)
        {
            ...
            return;
        }else if (!move_uploaded_file($_FILES['mp3']['tmp_name'],'mp3/'.$_FILES['mp3']['name']))
        {
    ...
    file_put_contents("music.txt",uniqid().'|'.$_POST["title"].'|'.$_POST["author"].'|'
        .'img/'.$_FILES['cover']['name'].'|'.'mp3/'.$_FILES['mp3']['name']."\n",FILE_APPEND);
